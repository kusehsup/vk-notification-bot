...
Бот крутится на нидерландском VPS (`5.129.227.157`), клиенты WireGuard выходят в интернет с того же адреса. Включи этот VPN, проверь IP на ifconfig.me, зайди на vk.ru и скопируй Bearer из `api.vk.ru/method/batch.call?client_id=6287487`. После отправки токена VPN можно выключить. Токен короткоживущий; когда протухнет — снять свежий тем же способом. Команда `/vpn`.

## Команды бота

| Команда | Что делает |
|---|---|
| `/start` | Инструкция + приём Bearer-токена |
...
| `/vpn` | Как выписать Bearer через WireGuard на IP бота |
