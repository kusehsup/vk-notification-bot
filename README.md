...
1. Открыть [vk.com](https://vk.com) в браузере и войти.
2. F12 → вкладка Network → обновить страницу.
3. Кликнуть любой запрос к vk.com → Request Headers → **Cookie**.
4. Прислать значение боту (должна быть строка с `remixsid=`). Если Telegram режет длину — сохранить в `cookies.txt` и прислать файлом.

Не копировать `document.cookie` из консоли: `remixsid` HttpOnly, его там нет.
